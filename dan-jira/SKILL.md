...
- **제목 형식**: `(M/DD) <핵심 정보>` — **모든 태스크**는 반드시 `(M/DD)` 접두사로 시작
  - 예: `(2/13) 서윤정님`, `(2/22) 챌린지 레이스`, `(2/14) 독서 - 원씽`
  - 날짜는 태스크를 수행하거나 완료할 예정 날짜 기준으로 설정
  - 날짜를 특정할 수 없는 경우에도 생성일 기준 `(M/DD)`를 붙일 것
- **본문(Description)**: 시간, 장소 등 상세 정보 기재 (예: `18:00 이수 방콕상회`)
- 시간 정보는 제목에 넣지 않고 본문에 넣기
- **dueDate 자동 설정** (우선순위):
  1. **제목에 날짜가 있는 경우**: `(M/DD)` 형식의 날짜를 `duedate`로 설정 (형식: `YYYY-MM-DD`)
     - 예: 제목이 `(2/13) 서윤정님`이면 → `"duedate": "2026-02-13"`
     - 현재 연도 기준으로 설정하되, 1~2월에 11~12월 날짜면 전년도로 간주
  2. **완료 상태로 전환하는 경우**: 제목에 날짜가 없으면, 완료로 옮긴 시점(오늘 날짜)을 `duedate`로 설정
