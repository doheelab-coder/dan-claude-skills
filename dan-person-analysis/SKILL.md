...
```
input/{사람명}/
├── 사진_1.png           # 본인 사진 (얼굴, 전신, 셀카 등)
├── 사진_2.png
├── 소개글_1.png         # 앱 프로필 캡처 (자기소개 텍스트)
├── 소개글_2.png         # 앱 프로필 캡처 (성격/취미/I have/I Want 등)
├── 대화_YYYY-MM-DD.csv  # 카카오톡 대화 내보내기 (날짜 포함)
...
입력 파일은 다음 3가지 카테고리로 분류하여 명명:

| 카테고리 | 파일명 패턴 | 설명 |
|---------|------------|------|
| **사진** | `사진_N.png` | 본인 사진 (얼굴, 전신, 셀카, 인바디 등) |
| **소개글** | `소개글_N.png` | 앱 프로필 텍스트 캡처 (자기소개, 성격/취미/관심사 등) |
| **대화** | `대화_YYYY-MM-DD.csv` | 카카오톡 대화 내보내기 (내보내기 날짜 포함) |

- 파일이 위 규칙과 다른 이름으로 되어 있으면, 이미지를 읽어 분류 후 규칙에 맞게 **자동 리네임** 수행
...
- **페이지 제목**: `{사람명} 프로필`
- **API 호출**: 항상 python3 urllib 사용 (MCP 도구 사용하지 않음)
- **인증**: `~/.claude/settings.json`의 `mcpServers.jira-personal.env.ATLASSIAN_API_TOKEN`
- **반드시 메인 컨텍스트에서 직접 실행**: 서브에이전트(Task)에 위임하지 않음 (Bash/Write 권한 승인 불가로 실패하므로)
