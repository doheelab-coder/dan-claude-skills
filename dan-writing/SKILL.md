...
모든 에픽은 `[구분자] 이름` 형식을 따릅니다. 새 에픽을 만들지 말고 기존 에픽에 태스크를 추가하세요.
...
태스크 추가 시 사용자가 말한 키워드로 적절한 에픽을 찾아 `parent`로 지정합니다.
...
- MCP 도구를 우선 사용하고, 실패 시 curl(python3 urllib)로 대안 시도
- 작업 완료 후 결과 URL을 반드시 반환
- 오류 발생 시 원인을 파악하고 가능한 대안을 시도
- Confluence 페이지 업데이트 시 **버전 번호를 반드시 1 증가**
- **에픽 하위 태스크 생성 시**: WebSearch로 관련 정보를 검색하여 본문(Description)에 포함
  - 예: 독서 에픽 → 책 제목으로 검색 → 저자, 출판사, 소개를 본문에 추가
- **태스크 생성 후 현재 활성 스프린트의 TODO에 자동 추가**
  - Board ID: 1, Agile API: `/rest/agile/1.0/board/1/sprint?state=active`로 활성 스프린트 조회
  - `/rest/agile/1.0/sprint/{sprintId}/issue`로 이슈 추가
